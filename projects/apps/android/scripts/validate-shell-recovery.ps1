...
$traceLog = Join-Path $companionDir "shell-recovery-$sessionTag.trace.log"
...
function Write-ValidationStep {
    ...
    $line = "[validate-shell-recovery] $Message"
    Write-Host $line
...
function Read-ValidationState {
    ...
    for ($attempt = 1; $attempt -le 5; $attempt++) {
        $jsonPath = Join-Path $env:TEMP ("makoion-shell-recovery-{0}.json" -f ([guid]::NewGuid()))
...
function Wait-ForState {
    ...
    throw "Timed out while waiting for $Description. Recent devices: $recentDevices Last transfer state: $lastTransfer Recent transfers: $recentTransfers"
}
...
function Validate-DelayedRetryRecovery {
    ...
    $scheduledRetryAt = Get-DelayedRetryScheduledAt -Transfer $queuedTransfer
